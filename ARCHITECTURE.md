...
| temporal smoothing | segmentation is independent per frame, so a still edge shimmers. Each mask takes a quarter of the new answer where the picture held still and nearly all of it where it moved (`K_STILL` 0.25, `K_MOVE` 0.97), judged by the frame's own colour over a tight motion band (`MOTION_LO` 0.025 … `MOTION_HI` 0.065). A fixed blend has to trade shimmer on a still shoulder against a smear on a moving hand: at a still edge the 0.6 it replaced let through about half of the model's shimmer (49–52% over three runs), this lets through about a quarter (≈20–29% depending on the run), and it follows movement as closely (IoU 0.97 against the truth, moving). The higher `K_MOVE` / tighter band is what keeps a head turn from trailing; nine tenths with a wider band left moderate turns near k ≈ 0.44 |
...
| person polish | with a background on, the lifted person gets a mild S-curve (`FG_CONTRAST` 0.12) and a light warm-midtone soften from a coarser mip (`FG_SOFTEN` 0.28), both alpha-gated so blur/stills are untouched. Contrast and soften scale down with `lowLight` so they do not stack on `LOW_LIGHT_CONTRAST` |
