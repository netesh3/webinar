...
func (s *Server) enqueueReminders(
	...
	jobs := []job{
		{types.NotifyReminder24h, "in 24 hours", starts.Add(-24 * time.Hour)},
		{types.NotifyReminder1h, "in 1 hour", starts.Add(-1 * time.Hour)},
	}
	now := time.Now()
	for _, j := range jobs {
		if !j.due.After(now) {
			// Window already passed (webinar is soon). Do not fire a "24h" mail at T-10m.
			continue
		}
		subject, body := notify.Reminder(in, j.window)
		if err := s.store.Notify(ctx, s.store.DB(), store.Notification{
			...
			s.log.Error("notify: could not queue reminder", "kind", j.kind, "email", email, "err", err)
		}
	}
}
